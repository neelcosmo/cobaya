...
from typing import Optional # changed by me: needed in BAO_CAMB
...
class BAO_CAMB(InstallableLikelihood):
    # Data type for aggregated chi2 (case sensitive)
    type = "BAO_CAMB"
    ...
    def get_requirements(self):
        ...
        theory_reqs = {
            ...
            "f_sigmas8": {
                "sigma_R": {"vars_pairs": (
                    [("v_newtonian_cdm", "v_newtonian_cdm")] + [("v_newtonian_baryon", "v_newtonian_baryon")]
                    + [("v_newtonian_cdm", "v_newtonian_baryon")]),
                    "z": zs.get("f_sigmas8", None),
                    "R": np.arange(1, 20, 0.5)},
                "Omega_b": {"z": 0},
                "Omega_cdm": {"z": 0},
    ...
    def theory_fun(self, z, observable):
        ...
        elif observable == "DH_over_DM":
            return (Const.c_km_s / self.provider.get_Hubble(z, units="km/s/Mpc")) / ((1 + z) * self.provider.get_angular_diameter_distance(z))
        # Comoving angular diameter distance, over sound horizon radius
        elif observable == "DM_over_rs":
            return (1 + z) * self.provider.get_angular_diameter_distance(z) / self.rs()
        # Physical angular diameter distance, over sound horizon radius
